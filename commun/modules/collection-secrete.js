...
const COLL_LABELS = ['', 'À vendre', 'Banal', 'Intéressant', 'Précieux', 'Inestimable'];

let collNewStars = 3;

const COLL_STATUT_STYLE = {
  'possédé': { color:'var(--GRN)',  label:'Sur soi'      },
  'sac':     { color:'var(--acc2)', label:'Dans le sac'  },
  'planque':  { color:'#8ba8c4',    label:'Cache secrète' },
  'perdu':    { color:'#c07070',    label:'Perdu'         },
  'donné':    { color:'var(--SUB)', label:'Donné'         },
};


/* Rendues accessibles aux boutons de la page. */
  window.coinsHTML = coinsHTML;
  window.toggleCacheDescForm = toggleCacheDescForm;
  window.addCollItem = addCollItem;
  window.deleteCollItem = deleteCollItem;
  window.renderCollection = renderCollection;
  window.renderCollStats = renderCollStats;
  window.renderCollNewStars = renderCollNewStars;
  window.updateCollStars = updateCollStars;
  window.updateCollStatut = updateCollStatut;

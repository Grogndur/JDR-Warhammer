...
function marqueEdit(i, field, val) {
  if(!viseeData.marques[i]) return;
  viseeData.marques[i][field] = val;
  viseeSave();
  const dot = document.querySelectorAll("#marques-list .marque-dot")[i];
  if(dot) dot.classList.toggle("active", (viseeData.marques[i].name || "").trim() !== "");
}

function marqueClear(i) {
  viseeData.marques[i] = { name:"", faille:"" };
  viseeSave();
  viseeRenderMarques();
}
...
/* Rendues accessibles aux boutons de la page. */
  window.carquoisAdd = carquoisAdd;
  window.carquoisQty = carquoisQty;
  window.carquoisDelete = carquoisDelete;
  window.souffleChange = souffleChange;
  window.posteAdd = posteAdd;
  window.posteDelete = posteDelete;
  window.viseeLoad = viseeLoad;
  window.viseeSave = viseeSave;
  window.viseeRenderSouffle = viseeRenderSouffle;
  window.viseeRenderMarques = viseeRenderMarques;
  window.viseeRenderPostes = viseeRenderPostes;
  window.viseeRenderCarquois = viseeRenderCarquois;
  window.viseeToggleForm = viseeToggleForm;
  window.viseeEsc = viseeEsc;
  window.marqueEdit = marqueEdit;
  window.marqueClear = marqueClear;

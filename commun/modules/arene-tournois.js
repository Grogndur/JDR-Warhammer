...
const TYPE_LABELS = { melee:'Mêlée', duel:'Duel', archery:'Tir à l\'arc', mixed:'Mixte' };


/* Rendues accessibles aux boutons de la page. */
  window.initPrestigePicker = initPrestigePicker;
  window.propagateWinner = propagateWinner;
  window.calcPlacement = calcPlacement;
  window.renderRenown = renderRenown;
  window.buildPodiumHTML = buildPodiumHTML;
  window.prestigeHTML = prestigeHTML;
  window.toggleWinner = toggleWinner;
  window.calcRenown = calcRenown;
  window.getGloryRank = getGloryRank;
  window.getMomentumTier = getMomentumTier;
  window.clearPropagation = clearPropagation;
  window.arenaSave = arenaSave;
  window.arenaTab = arenaTab;
  window.buildBracket = buildBracket;
  window.createTourney = createTourney;
  window.deleteTourney = deleteTourney;
  window.editBracketSlot = editBracketSlot;
  window.editPFSlot = editPFSlot;
  window.finishTourney = finishTourney;
  window.getActiveTourney = getActiveTourney;
  window.renderActiveBracket = renderActiveBracket;
  window.renderTourneyHistory = renderTourneyHistory;
  window.togglePFWinner = togglePFWinner;
